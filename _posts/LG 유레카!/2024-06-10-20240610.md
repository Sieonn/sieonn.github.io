...
#### 4. 구성요소 사용법
 ...
 - 클래스
   ...
   class 클래스명<br/>{<br/>//변수 선언<br/>
   //메서드 정의<br/>
   }
   <span style="color:red">void hello()<br>{ }</span>
...
{<br>
   ...
   **---> 메서드정의**<br>
   리턴자료형 메서드명(매개변수 선언)<br>
   {<br>
      ---> 변수선언<small>`local variable != field`</small><br>
      ---> 변수초기화<br>
      ---> **메서드호출**<br>
      ---> 조건문, 반복문<br>
   }<br>
...
문제4) 다음과 같이 화면에 출력하시오.
  ...
  for(int i=3; i<=12; **i+=3**){  <small>//(i=i+3) == i+=3</small>
     System.out.print(i + " ");
  }
