...
1. **민감한 데이터 보호**
   - `/admin/`, `/login/`, `/config/` 같은 민감한 페이지를 검색 엔진에 노출하지 않으려는 경우.
2. **불필요한 크롤링 방지**
   - 이미지, PDF, 임시 파일, 테스트 페이지 등 검색 엔진에 표시될 필요가 없는 리소스를 크롤링하지 않도록 설정.
3. **중복 콘텐츠 방지**
   - 필터된 페이지나 정렬된 페이지와 같은 중복 콘텐츠를 검색 엔진이 크롤링하지 못하도록 제한.
4. **SEO 최적화**
   - 검색 엔진이 중요한 페이지(예: 제품 페이지, 블로그 포스트 등)만 크롤링하도록 리소스를 집중.
5. **크롤러 블록**
   - 사이트 성능에 영향을 줄 수 있는 악의적이거나 과도한 크롤링을 차단.
...
1. **`Disallow`와 `Allow` 사용법**
   ...
   - ```
     ...
     를 사용하여 특정 파일은 허용하고 나머지는 금지할 수 있습니다:
...
<img src="/../images/2025-01-05-빌드/image-20250105024010276.png" alt="image-20250105024010276" style="zoom:80%;" />
